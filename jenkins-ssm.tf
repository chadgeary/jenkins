resource "aws_ssm_document" "jenkins-ssm-doc" {
  ...
  {
    "schemaVersion": "2.2",
    "description": "Ansible Playbooks via SSM for Ubuntu 18.04, installs Ansible properly.",
    "parameters": {
    "SourceType": {
...
resource "aws_ssm_association" "jenkins-master-ssm-assoc" {
  ...
  parameters              = {
    ExtraVariables          = "SSM=True s3_bucket=${aws_s3_bucket.jenkins-bucket.id} project_prefix=${var.project_prefix} docker_network=${var.docker_network} docker_gw=${var.docker_gw} docker_jenkinsmaster=${var.docker_jenkinsmaster} docker_webproxy=${var.docker_webproxy}"
